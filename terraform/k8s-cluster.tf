  ...
  instance_template {
    ...
    boot_disk {
      initialize_params {
        ...
        size     = 40
        type     = "network-hdd"
        #network-ssd, network-hdd, network-ssd-nonreplicated
      }
    }
...
resource "yandex_lb_network_load_balancer" "k8s-load-balancer" {
  ...
  listener {
    ...
    external_address_spec {
      #адрес был предварительно зарезервирован
      address ="51.250.8.59"
      ip_version = "ipv4"
    }
  }
...
# resource "yandex_storage_bucket" "backup-backet-apatsev1" {
#   bucket = "backup-backet-apatsev1"
#   force_destroy = true
